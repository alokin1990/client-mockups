(() => {
  ...
  const measureFirstScreen = () => {
    ...
    document.querySelectorAll(heroSelector).forEach((hero) => {
      ...
      const availableHeight = Math.max(0, window.innerHeight - documentTop - tailHeight);
      hero.style.setProperty("--first-screen-offset", `${Math.max(0, documentTop).toFixed(2)}px`);
      hero.style.setProperty("--first-screen-tail-size", `${tailHeight.toFixed(2)}px`);

      let fitted = false;
      for (const density of densityLevels) {
        hero.setAttribute("data-hero-density", density);
        fitted = hero.getBoundingClientRect().height <= availableHeight + 1
  ...
  if ("ResizeObserver" in window) {
    ...
    document.querySelectorAll(`${chromeSelector}, ${tailSelector}`).forEach((element) => {
      firstScreenObserver.observe(element);
    });
  }
})();
